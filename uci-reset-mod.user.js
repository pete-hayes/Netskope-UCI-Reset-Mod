...
// @version      1.1
// @description  Adds a "Reset UCI Score" link beside the active user in the Netskope UBA Incidents dashboard.
// @author       Peter Hayes
...
(() => {
    ...
    const remove = () =>
        document.querySelectorAll('.uci-wrapper').forEach(e => e.remove());
    ...
    const inject = () => {
        ...
        const span = document.querySelector('.user-header-info .ns-txt-page-sub-title');
        if (!span) return;
